...
/// <summary>
/// PPF/Vehicle Protection Pricing view with template-based customization.
/// - Original templates are read-only
/// - Users can "Make a Copy" to create editable versions
/// - Edit mode allows changing labels, adding/removing services
/// </summary>
public class PPFPricingView : UserControl
{
    ...
    private void BuildUI()
    {
        ...
        _formBuilder = new TemplateFormBuilder(ShopDocType.PPFPricing);
        ...
        exportContent.Children.Add(new TextBlock { Text = "Generate Quote PDF", VerticalAlignment = VerticalAlignment.Center });
    ...
    private void OnExportClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var pdfData = new VehicleProtectionPdfData
            {
                Date = DateTime.Now,
                ServiceType = "ppf",
                ServiceTypeName = "Paint Protection Film",
                CustomerName = GetStringValue(data, "customerName"),
                VehicleStyle = GetStringValue(data, "vehicleYMM"),
                ProductName = GetStringValue(data, "productName")
            };

            // Add selected services as panels
            if (data.ContainsKey("charges") && data["charges"] is List<(string Name, decimal Amount)> charges)
            {
                ...
                pdfData.PanelCount = charges.Count;
            }
            ...
            // Track usage
            DocumentUsageTrackingService.Instance.RecordPdfExport("PPFQuote", Path.GetFileName(pdfPath), 1);
            ...
            ShowNotification("PPF Quote exported successfully!", InfoBarSeverity.Success);
        }
        catch (Exception ex)
        {
