...
namespace McStudDesktop.Views
{
    ...
    public sealed class ScreenMonitorPanel : UserControl
    {
        ...
        private StackPanel? _activeSuggestionPanel; // currently expanded per-op suggestion panel
        ...
        private StackPanel CreateHeader()
        {
            ...
            _insuranceButton = new Button
            {
                Content = new StackPanel
                {
                    ...
                    {
                        new FontIcon { Glyph = "\uE8D4", FontSize = 14 },
                        new TextBlock { Text = "Insurance", FontSize = 12, FontWeight = Microsoft.UI.Text.FontWeights.SemiBold }
                    }
                },
            ...
            ToolTipService.SetToolTip(_insuranceButton,
                "Select an insurer DRP profile to check compliance as you estimate.\n" +
                "The scrubber will flag issues before you submit.");
            _insuranceButton.Click += async (s, e) => await ShowInsuranceProfileSelector();
            Grid.SetColumn(_insuranceButton, 1);
            titleRow.Children.Add(_insuranceButton);
        ...
        private async Task ShowInsuranceProfileSelector()
        {
            ...
            // Active profile info
            var activeRow = new Border
            {
                Background = new SolidColorBrush(Color.FromArgb(255, 30, 25, 45)),
                CornerRadius = new CornerRadius(6),
                Padding = new Thickness(10, 8, 10, 8),
                Visibility = _activeInsurerProfile != null ? Visibility.Visible : Visibility.Collapsed
            };
            var activeText = new TextBlock
            {
                Text = _activeInsurerProfile != null
                    ? $"Active: {_activeInsurerProfile.CompanyName} — {_activeInsurerProfile.ProgramName}"
                    : "",
                FontSize = 12,
                FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
                Foreground = new SolidColorBrush(Color.FromArgb(255, 200, 160, 240))
            };
            ...
            void RefreshProfileList()
            {
                ...
                if (profiles.Count == 0)
                {
                    profilesStack.Children.Add(new TextBlock
                    {
                ...
                {
                    var capturedP = p;
                    var isSelected = selectedProfile?.Id == p.Id;

                    var row = new Border
                    {
                        Background = new SolidColorBrush(isSelected
                            ? Color.FromArgb(255, 55, 35, 80)
                            : Color.FromArgb(255, 35, 38, 48)),
                        CornerRadius = new CornerRadius(6),
                        Padding = new Thickness(10, 8, 10, 8),
                    ...
                    rowStack.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });
                    ...
                    if (!string.IsNullOrEmpty(p.ProgramName))
                    {
                        infoStack.Children.Add(new TextBlock
                        {
                            Text = p.ProgramName,
                            FontSize = 11,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 160, 130, 200))
                        });
                    }
                    var metaText = $"{p.ChecklistItems.Count} checklist items";
                    if (p.AgreedRates.Count > 0)
                        metaText += $"  ·  {p.AgreedRates.Count} rate agreements";
                    if (p.IsExample)
                        metaText += "  ·  Example profile";
                    infoStack.Children.Add(new TextBlock
                    {
                        Text = metaText,
                        FontSize = 10,
                        Foreground = new SolidColorBrush(Color.FromArgb(255, 110, 110, 130))
                    });
                    ...
                    if (!p.IsExample)
                    {
                        ...
                        ToolTipService.SetToolTip(deleteBtn, "Delete profile");
                        deleteBtn.Click += (s, e) =>
                        {
                            service.Remove(capturedP.Id);
                            if (selectedProfile?.Id == capturedP.Id)
                    ...
                    rowStack.Children.Add(actionStack);

                    row.Child = rowStack;

                    row.PointerPressed += (s, e) =>
                    {
                        selectedProfile = capturedP;
                        activeText.Text = $"Active: {capturedP.CompanyName} — {capturedP.ProgramName}";
                        activeRow.Visibility = Visibility.Visible;
                        RefreshProfileList();
                    };

                    profilesStack.Children.Add(row);
                }
            }
            ...
            clearBtn.Click += (s, e) =>
            {
                selectedProfile = null;
                activeText.Text = "";
                activeRow.Visibility = Visibility.Collapsed;
            ...
            {
                Height = 1,
                Background = new SolidColorBrush(Color.FromArgb(255, 55, 55, 70)),
                Margin = new Thickness(0, 4, 0, 4)
            });
            ...
            var companyInput = new TextBox
            {
                PlaceholderText = "Company name (e.g. Allstate)",
                FontSize = 12,
                Margin = new Thickness(0, 0, 6, 0)
            };
            Grid.SetColumn(companyInput, 0);

            var programInput = new TextBox
            {
                PlaceholderText = "Program name (e.g. PRO Program)",
                FontSize = 12,
                Margin = new Thickness(0, 0, 6, 0)
            };
            Grid.SetColumn(programInput, 1);
            ...
            var addProfileBtn = new Button
            {
                ...
                Background = new SolidColorBrush(Color.FromArgb(255, 70, 40, 100)),
                Foreground = new SolidColorBrush(Colors.White),
                CornerRadius = new CornerRadius(4)
            };
            addProfileBtn.Click += (s, e) =>
            {
                var company = companyInput.Text?.Trim();
                var program = programInput.Text?.Trim();
                if (string.IsNullOrEmpty(company)) return;
                service.Add(company, program ?? "DRP Program");
                companyInput.Text = "";
                programInput.Text = "";
                RefreshProfileList();
            };
            Grid.SetColumn(addProfileBtn, 2);

            addGrid.Children.Add(companyInput);
            addGrid.Children.Add(programInput);
            addGrid.Children.Add(addProfileBtn);
            dialogContent.Children.Add(addGrid);

            var dialog = new ContentDialog
            {
                Title = "Insurance / DRP Profile",
            ...
            if (result == ContentDialogResult.Primary)
            {
                ...
                // Rebuild display to show/hide the DRP compliance section
                if (_latestSnapshot != null || _latestResult != null)
                    RebuildAnalysisDisplay();
            }
        }
        ...
        private void RebuildAnalysisDisplay()
        {
            ...
            // === DRP COMPLIANCE SECTION ===
            if (_activeInsurerProfile != null)
            {
                try
                {
                    var rawText = result?.RawText ?? "";
                    var compliance = InsurerProfileService.Instance.CheckCompliance(_activeInsurerProfile, rawText);
                    var notMet = compliance.Where(c => !c.IsMet).ToList();
                    var met = compliance.Where(c => c.IsMet).ToList();

                    var drpBorder = new Border
                    {
                        Background = new SolidColorBrush(Color.FromArgb(255, 28, 35, 50)),
                        CornerRadius = new CornerRadius(6),
                        Padding = new Thickness(10, 8, 10, 8),
                        Margin = new Thickness(0, 4, 0, 2),
                        BorderBrush = new SolidColorBrush(notMet.Count > 0
                            ? Color.FromArgb(255, 160, 80, 180)
                            : Color.FromArgb(255, 50, 120, 180)),
                        BorderThickness = new Thickness(1)
                    };
                    var drpStack = new StackPanel { Spacing = 4 };

                    // Title row
                    var drpTitleRow = new Grid();
                    drpTitleRow.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
                    drpTitleRow.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });

                    var drpTitleStack = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 6 };
                    drpTitleStack.Children.Add(new FontIcon
                    {
                        Glyph = "\uE8D4",
                        FontSize = 11,
                        Foreground = new SolidColorBrush(Color.FromArgb(255, 180, 140, 220))
                    });
                    drpTitleStack.Children.Add(new TextBlock
                    {
                        Text = $"DRP COMPLIANCE — {_activeInsurerProfile.CompanyName.ToUpperInvariant()}",
                        FontSize = 12,
                        FontWeight = Microsoft.UI.Text.FontWeights.Bold,
                        Foreground = new SolidColorBrush(Color.FromArgb(255, 200, 160, 240))
                    });
                    Grid.SetColumn(drpTitleStack, 0);
                    drpTitleRow.Children.Add(drpTitleStack);

                    var drpScoreText = new TextBlock
                    {
                        Text = $"{met.Count}/{compliance.Count}",
                        FontSize = 12,
                        FontWeight = Microsoft.UI.Text.FontWeights.Bold,
                        Foreground = new SolidColorBrush(notMet.Count > 0
                            ? Color.FromArgb(255, 220, 140, 255)
                            : Color.FromArgb(255, 80, 200, 120))
                    };
                    Grid.SetColumn(drpScoreText, 1);
                    drpTitleRow.Children.Add(drpScoreText);
                    drpStack.Children.Add(drpTitleRow);

                    if (!string.IsNullOrEmpty(_activeInsurerProfile.ProgramName))
                    {
                        drpStack.Children.Add(new TextBlock
                        {
                            Text = _activeInsurerProfile.ProgramName,
                            FontSize = 10,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 140, 120, 180)),
                            Margin = new Thickness(0, 0, 0, 2)
                        });
                    }

                    // Group by category
                    var drpGrouped = compliance.GroupBy(c => c.Item.Category).ToList();
                    foreach (var grp in drpGrouped)
                    {
                        drpStack.Children.Add(new TextBlock
                        {
                            ...
                            FontSize = 10,
                            FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 130, 120, 160)),
                            Margin = new Thickness(0, 4, 0, 1)
                        });

                        foreach (var (item, isMet, detail) in grp)
                        {
                            var itemRow = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 6 };
                            itemRow.Children.Add(new TextBlock
                            {
                                Text = isMet ? "\u2705" : (item.EstimateKeywords == null ? "\u26A0\uFE0F" : "\u274C"),
                                FontSize = 11,
                                VerticalAlignment = VerticalAlignment.Top,
                                Margin = new Thickness(0, 1, 0, 0)
                            });
                            var itemInner = new StackPanel { Spacing = 1 };
                            itemInner.Children.Add(new TextBlock
                            {
                                Text = item.Description,
                                FontSize = 11,
                                Foreground = new SolidColorBrush(isMet
                                    ? Color.FromArgb(255, 100, 200, 120)
                                    : (item.EstimateKeywords == null
                                        ? Color.FromArgb(255, 200, 180, 100)
                                        : Color.FromArgb(255, 220, 130, 80))),
                                TextWrapping = TextWrapping.Wrap,
                                TextDecorations = isMet ? Windows.UI.Text.TextDecorations.Strikethrough : Windows.UI.Text.TextDecorations.None
                            });
                            ...
                            itemRow.Children.Add(itemInner);
                            drpStack.Children.Add(itemRow);
                        }
                    }
                    ...
                    if (notMet.Count == 0 && compliance.Count > 0)
                    {
                        drpStack.Children.Add(new TextBlock
                        {
                            Text = "All detectable items confirmed \u2014 verify manual items before submitting.",
                            FontSize = 10,
                            FontStyle = Windows.UI.Text.FontStyle.Italic,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 80, 180, 120)),
                            Margin = new Thickness(0, 4, 0, 0)
                        });
                    }

                    drpBorder.Child = drpStack;
                    _analysisContentStack.Children.Add(drpBorder);
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"[ScreenMonitor] DRP compliance error: {ex.Message}");
                }
            }
            ...
            foreach (var sectionName in orderedSections)
            {
                ...
                // Estimate operations for this section (clickable to show suggestions from prior estimates)
                if (hasEstOps)
                {
                    var vehicleInfo = snapshot?.VehicleInfo;
                    foreach (var op in sectionOps!)
                    {
                        var opContainer = new StackPanel { Spacing = 0 };
                        var row = CreateAccumulatedOperationRow(op, confirmedTitles);
                        opContainer.Children.Add(row);

                        // Collapsible suggestion panel (hidden by default)
                        var suggPanel = new StackPanel
                        {
                            Visibility = Visibility.Collapsed,
                            Spacing = 2,
                            Margin = new Thickness(8, 0, 0, 4),
                            Padding = new Thickness(8, 6, 8, 6),
                            Background = new SolidColorBrush(Color.FromArgb(255, 25, 35, 42))
                        };
                        opContainer.Children.Add(suggPanel);

                        var capturedOp = op;
                        var capturedPanel = suggPanel;
                        row.PointerPressed += (s, e) =>
                        {
                            ToggleOpSuggestionPanel(capturedOp, capturedPanel, vehicleInfo);
                        };

                        contentPanel.Children.Add(opContainer);
                    }
                }
        ...
        private List<MustHaveChecklistItem> BuildScreenMustHaveChecklist(IReadOnlyList<ParsedEstimateLine> accumulatedOps)
        {
            ...
            // Filter by vehicle fuel type context when available (insurance company not available in screen read)
            var vehicleInfo = coaching.LatestSnapshot?.VehicleInfo;
            var vehicleFuelType = PatternIntelligenceService.Instance.ClassifyVehicleFuelType(vehicleInfo);
            var mustHaves = GhostConfigService.Instance.GetMustHavesForContext(null, vehicleFuelType);
            ...
            foreach (var mh in mustHaves)
            {
                ...
                // Use strict mode: the raw blob contains CCC section headers and UI text,
                // so require ALL significant words to match to avoid false positives
                // (e.g., "Steering Wheel Lock" matching "STEERING WHEEL 2021-25 W/LANE KEEP").
                if (!isPresent && rawOcrNormalized.Length > 0)
                    isPresent = MatchesMustHave(rawOcrNormalized, opNorm, opWords, strictMode: true);
        ...
        /// </summary>
        private static bool MatchesMustHave(string detectedNorm, string mustHaveNorm, string[] mustHaveWords, bool strictMode = false)
            => GhostConfigService.MatchesMustHave(detectedNorm, mustHaveNorm, mustHaveWords, strictMode);
        ...
        /// <summary>
        /// Toggle the inline suggestion panel for a clicked accumulated operation.
        /// Queries SmartSuggestionService + co-occurrence data from prior estimates.
        /// </summary>
        private void ToggleOpSuggestionPanel(ParsedEstimateLine op, StackPanel panel, string? vehicleInfo)
        {
            // If this panel is already open, just collapse it
            if (panel.Visibility == Visibility.Visible)
            {
                panel.Visibility = Visibility.Collapsed;
                if (_activeSuggestionPanel == panel) _activeSuggestionPanel = null;
                return;
            }

            // Collapse any previously open panel
            if (_activeSuggestionPanel != null && _activeSuggestionPanel != panel)
                _activeSuggestionPanel.Visibility = Visibility.Collapsed;

            _activeSuggestionPanel = panel;
            panel.Children.Clear();

            try
            {
                var partName = op.PartName ?? op.Description ?? "";
                var opType = op.OperationType ?? "";

                // Query learned suggestions
                var suggestions = SmartSuggestionService.Instance.GetSuggestionsForPart(partName, opType, vehicleInfo);

                // Also query co-occurring operations
                var coOccurrences = EstimateLearningService.Instance.GetRelatedOperations(partName, opType, maxResults: 8);

                var hasAnything = suggestions.HasData || coOccurrences.Count > 0;

                if (!hasAnything)
                {
                    panel.Children.Add(new TextBlock
                    {
                        Text = "No prior estimate data for this operation yet.",
                        FontSize = 10,
                        FontStyle = Windows.UI.Text.FontStyle.Italic,
                        Foreground = new SolidColorBrush(Color.FromArgb(255, 120, 120, 140))
                    });
                ...
                {
                    Text = "SUGGESTED FROM PRIOR ESTIMATES",
                    FontSize = 10,
                    FontWeight = Microsoft.UI.Text.FontWeights.Bold,
                    Foreground = new SolidColorBrush(Color.FromArgb(255, 100, 200, 180)),
                    VerticalAlignment = VerticalAlignment.Center
                };
                Grid.SetColumn(headerText, 0);
                headerRow.Children.Add(headerText);

                if (suggestions.MatchCount > 0)
                {
                    ...
                    Grid.SetColumn(matchInfo, 1);
                    headerRow.Children.Add(matchInfo);
                }
                panel.Children.Add(headerRow);

                // Manual operations (additional ops commonly seen with this part)
                if (suggestions.ManualOperations.Count > 0)
                {
                    foreach (var sugOp in suggestions.ManualOperations.Take(8))
                    {
                        panel.Children.Add(BuildSuggestionRow(sugOp, suggestions.MatchCount));
                    }
                }

                // Category-pooled suggestions (from similar parts)
                if (suggestions.CategoryOperations.Count > 0)
                {
                    ...
                    foreach (var catOp in suggestions.CategoryOperations.Take(5))
                    {
                        panel.Children.Add(BuildSuggestionRow(catOp, suggestions.MatchCount));
                    }
                }

                // Co-occurring operations (other parts/ops commonly on the same estimate)
                var filteredCoOcc = coOccurrences
                    .Where(c => c.CoOccurrenceRate >= 0.3) // only show if seen ≥30% of the time
                    .ToList();
                if (filteredCoOcc.Count > 0)
                {
                    panel.Children.Add(new TextBlock
                    {
                        Text = "COMMONLY PAIRED OPERATIONS",
                        FontSize = 9,
                        FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
                        Foreground = new SolidColorBrush(Color.FromArgb(255, 180, 160, 100)),
                        Margin = new Thickness(0, 6, 0, 2)
                    });

                    foreach (var coOp in filteredCoOcc)
                    {
                        var coRow = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 8, Margin = new Thickness(4, 1, 0, 1) };
                        coRow.Children.Add(new TextBlock
                        {
                            Text = "\u2022",
                            FontSize = 10,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 180, 160, 100)),
                        ...
                        var coDesc = $"{coOp.OperationType} {coOp.PartName}";
                        coRow.Children.Add(new TextBlock
                        {
                            Text = coDesc,
                            FontSize = 10,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 200, 195, 170)),
                            VerticalAlignment = VerticalAlignment.Center,
                            TextTrimming = TextTrimming.CharacterEllipsis
                        });
                        ...
                        coRow.Children.Add(new TextBlock
                        {
                            Text = $"{coOp.CoOccurrenceRate:P0}",
                            FontSize = 9,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 140, 130, 90)),
                            VerticalAlignment = VerticalAlignment.Center
                        });

                        if (coOp.AvgLaborHours > 0)
                        {
                            coRow.Children.Add(new TextBlock
                            {
                                Text = $"{coOp.AvgLaborHours:N1}h",
                                FontSize = 9,
                        ...
                        panel.Children.Add(coRow);
                    }
                }

                // Summary: potential value
                if (suggestions.HasData)
                {
                    suggestions.CalculateTotals();
                    var parts = new List<string>();
                    if (suggestions.TotalLaborHours > 0) parts.Add($"{suggestions.TotalLaborHours:N1}h labor");
                    if (suggestions.TotalRefinishHours > 0) parts.Add($"{suggestions.TotalRefinishHours:N1}h refinish");
                    if (suggestions.EstimatedValue > 0) parts.Add($"${suggestions.EstimatedValue:N2}");

                    if (parts.Count > 0)
                    {
                        panel.Children.Add(new TextBlock
                        {
                            Text = $"Potential: {string.Join(" + ", parts)}",
                            FontSize = 9,
                            FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
                            Foreground = new SolidColorBrush(Color.FromArgb(255, 100, 200, 180)),
                            Margin = new Thickness(0, 4, 0, 0)
            ...
            {
                System.Diagnostics.Debug.WriteLine($"[ScreenMonitor] Suggestion panel error: {ex.Message}");
                panel.Children.Add(new TextBlock
                {
        ...
        /// <summary>
        /// Build a single row for a suggested operation (manual line / additional op).
        /// </summary>
        private static Border BuildSuggestionRow(SmartSuggestionOp sugOp, int matchCount)
        {
            var row = new Border
            {
                Background = new SolidColorBrush(Color.FromArgb(255, 30, 40, 48)),
                CornerRadius = new CornerRadius(3),
            ...
            // Frequency badge
            if (sugOp.TimesUsed > 0 && matchCount > 0)
            {
                leftStack.Children.Add(new TextBlock
                {
                    Text = $"{sugOp.TimesUsed}/{matchCount}",
                    FontSize = 9,
                    Foreground = new SolidColorBrush(Color.FromArgb(255, 100, 160, 220)),
                    VerticalAlignment = VerticalAlignment.Center
                });
            ...
            if (sugOp.LaborHours > 0)
            {
                rightStack.Children.Add(new TextBlock
                {
                    Text = $"{sugOp.LaborHours:N1}h",
                    FontSize = 9,
                    Foreground = new SolidColorBrush(Color.FromArgb(255, 100, 180, 130)),
                    VerticalAlignment = VerticalAlignment.Center
                });
            }
            if (sugOp.RefinishHours > 0)
            {
                rightStack.Children.Add(new TextBlock
                {
                    Text = $"{sugOp.RefinishHours:N1}r",
                    FontSize = 9,
                    Foreground = new SolidColorBrush(Color.FromArgb(255, 170, 140, 240)),
                    VerticalAlignment = VerticalAlignment.Center
                });
            }
            if (sugOp.Price > 0)
            {
                rightStack.Children.Add(new TextBlock
                {
                    ...
                    FontSize = 9,
                    FontWeight = Microsoft.UI.Text.FontWeights.SemiBold,
                    Foreground = new SolidColorBrush(Color.FromArgb(255, 230, 190, 90)),
                    VerticalAlignment = VerticalAlignment.Center
                });
            }
