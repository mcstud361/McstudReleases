...
public class AuthenticationService
{
    ...
    {
        // Load users synchronously in constructor - this is fast enough for small user files
        // This prevents race conditions and deadlocks
        LoadUsers();
        EnsureDefaultAdminExists();
        _isInitialized = true;
        System.Diagnostics.Debug.WriteLine("[Auth] AuthenticationService initialized in constructor");
    }

    public async Task InitializeAsync()
    {
        // Already initialized in constructor, this is now a no-op
    ...
    public bool ValidateCredentials(string username, string password)
    {
        ...
        if (user.PasswordHash == passwordHash)
        {
            ...
            _ = SaveUsersAsync(); // Fire and forget - don't block login
            return true;
        }
    ...
    private async Task LoadUsersAsync()
    {
        try
        {
            if (File.Exists(UsersFilePath))
            {
                var encryptedData = await File.ReadAllBytesAsync(UsersFilePath);
                var decryptedData = await Task.Run(() => DecryptData(encryptedData));
                var json = Encoding.UTF8.GetString(decryptedData);
                _users = JsonSerializer.Deserialize<List<UserAccount>>(json) ?? new List<UserAccount>();
                System.Diagnostics.Debug.WriteLine($"[Auth] Loaded {_users.Count} users from encrypted file (async)");
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task SaveUsersAsync()
    {
        try
        {
            var json = JsonSerializer.Serialize(_users, new JsonSerializerOptions { WriteIndented = true });
            var dataBytes = Encoding.UTF8.GetBytes(json);
            var encryptedData = await Task.Run(() => EncryptData(dataBytes));
            await File.WriteAllBytesAsync(UsersFilePath, encryptedData);
            System.Diagnostics.Debug.WriteLine($"[Auth] Saved {_users.Count} users to encrypted file (async)");
        }
        catch (Exception ex)
        {
            System.Diagnostics.Debug.WriteLine($"[Auth] Error saving users: {ex.Message}");
